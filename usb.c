...
/*
 * usb_open_attempt() - one libusb scan-and-open pass.
 *
 ...
 */
int usb_open_attempt(struct qdl_device *qdl, const char *serial, int *visible_out)
{
...
static int usb_open(struct qdl_device *qdl, const char *serial)
{
	...
	for (;;) {
		ret = usb_open_attempt(qdl, serial, &visible);
		if (ret == 0)
			return 0;
		if (ret == -EIO)
			return -1;
