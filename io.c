...
int qdl_push_back(struct qdl_device *qdl, const void *buf, size_t len)
{
	...
	if (!qdl->pending_buf) {
		qdl->pending_buf = malloc(len);
		if (!qdl->pending_buf)
			return -ENOMEM;
		memcpy(qdl->pending_buf, buf, len);
		qdl->pending_len = len;
		qdl->pending_off = 0;
		return 0;
	}

	leftover = qdl->pending_len - qdl->pending_off;
	grown = malloc(leftover + len);
	if (!grown)
		return -ENOMEM;
	memcpy(grown, qdl->pending_buf + qdl->pending_off, leftover);
	memcpy(grown + leftover, buf, len);
	free(qdl->pending_buf);
	qdl->pending_buf = grown;
	qdl->pending_len = leftover + len;
	qdl->pending_off = 0;
	return 0;
}
